...
class HGCEnv:
    ...
    def __init__(self, ged_method='', method_arguments='', use_custom_edit_costs=False, init_type=''):
        if use_custom_edit_costs and not edit_costs.initialized:
            raise Exception("Custom edit costs were activated but not set up before. Use hgc_env.set_custom_edit_costs(func, func, func, func, func, func).")
        try:
            self._hgcdicodaged = HGCDiCoDaGED.HGCDiCoDaGED(ged_method, method_arguments, use_custom_edit_costs, init_type)
            ...
            raise ModuleNotFoundError("Couldn't find edit_costs.py file! Please stick to the following file structure:\n"
                                      ...
                                      "\t- main.py\n"
                                      "\t- ...")
        self._edit_costs = self._hgcdicodaged.get_edit_costs_name()
        self._ged_method = self._hgcdicodaged.get_method_name()

    def set_ged_method(self, ged_method, method_arguments=''):
        self._hgcdicodaged.set_method(ged_method, method_arguments)
        self._ged_method = self._hgcdicodaged.get_method_name()

    def load_csv(self, omics_dataset, clinical_dataset='', use_csv_edit_costs=False, costs_dataset='', seperator=','):
        print('Loading CSV data... ', end='')
        self._hgcdicodaged.load_csv(omics_dataset, clinical_dataset, use_csv_edit_costs, costs_dataset, seperator)
    ...
    def generate_labels(self, labeled_variable=''):
        ...
        self._hgcdicodaged.generate_labels(labeled_variable)
        self._label_list = self._hgcdicodaged.get_label_vector()
    ...
    def compute_geds(self):
        ...
        self._hgcdicodaged.compute_geds()
        self._distance_matrix = self._hgcdicodaged.get_distance_matrix()
    ...
    def pull_graph(self, graph_id):
        return glnx_parser.pull_graph(self._hgcdicodaged, graph_id)
    ...
    def push_graph(self, graph, name, node_label_key='', edge_label_key=''):
        ...
        return glnx_parser.push_graph(self._hgcdicodaged, graph, name, node_label_key, edge_label_key)

    def export_environment(self, out_dir):
        print('Exporting environment to \"' + out_dir + '\"... ', end='')
        glnx_parser.export_environment(self._hgcdicodaged, out_dir)
        print('Done!')
    ...
    def import_environment(self, directory, node_label_key='', edge_label_key=''):
        ...
        glnx_parser.import_environment(self._hgcdicodaged, directory, node_label_key, edge_label_key)
        print('Done!')
